...
class WebSocketEvent
{
    ...
    public function onHandShake(\swoole_http_request $request, \swoole_http_response $response)
    {
        ...
        $fd = $this->secWebsocketAccept($request, $response);
        if ($fd) {
            $arr = Redis::getInstance()->get("ws_key");
            $arr = json_decode($arr,true) ?? [];
            array_push($arr,$fd);
            Redis::getInstance()->set("ws_key",json_encode($arr));
            $response->end();
            return true;
        }
        $response->end();
     ...
     * @return bool
     */
    protected function secWebsocketAccept(\swoole_http_request $request, \swoole_http_response $response)
    {
        // ws rfc 规范中约定的验证过程
